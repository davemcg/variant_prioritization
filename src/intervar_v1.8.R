...
intervar_for_sorting <- intervar %>% 
  ...
  mutate(truncating = case_when(PVS1 == 0 & grepl("^frameshift|stop", ExonicFunc.refGene, ignore.case = TRUE) & maxaf_intervar < 0.005  ~ 6,
                                PVS1 == 0 & grepl("^frameshift|stop", ExonicFunc.refGene, ignore.case = TRUE) & maxaf_intervar < 0.02  ~ 3,
                                TRUE ~ 0)) %>% 
  mutate(BS1 = ifelse(Freq_gnomAD_genome_ALL < 0.005 & Freq_esp6500siv2_all < 0.01, 0, BS1)) %>% 
  mutate(Priority.Score = (PVS1*8+truncating+(PS1+PS2+PS3+PS4+PS5)*6+(PM1+PM2+PM3+PM4+PM5+PM6+PM7)*3+(PP1+PP2+PP3+PP4+PP5+PP6)-BA1*5-(BS1+BS2+BS3+BS4+BS5)*3-(BP1+BP2+BP3+BP4+BP5+BP6+BP7+BP8))) %>% 
  unite("variantkey", X.Chr:Alt, sep = "_", remove = FALSE ) %>%
  group_by(variantkey) %>%
  slice(which.max(Priority.Score))
#intervar 2.1.2 use the following for BS1 (0.005 cutoff): {'1000g2015aug_all':0,'esp6500siv2_all':0,'gnomAD_genome_ALL':0,'gnomAD_genome_AFR':0,'gnomAD_genome_AMR':0,'gnomAD_genome_EAS':0,'gnomAD_genome_FIN':0,'gnomAD_genome_NFE':0,'gnomAD_genome_OTH':0,'gnomAD_genome_ASJ':0}  
