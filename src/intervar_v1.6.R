...
## if PVS = 0 & |dpsi_max_tissue+dpsi_zscore| > 2.5, score=+2, make a histogram of dpsi of WGS set and determine the cut-off
## if PVS = 0 & spliceai_filtered > 0.1, score=+2, make a histogram of splicai score and determine the cut-off
...
intervar_for_sorting <- intervar %>% 
  ...
  mutate(dbscSNV_value = ifelse((PVS1 == 0 & dbscSNV_ADA_SCORE>0.8 & dbscSNV_RF_SCORE>0.5), 1, 0)) %>% 
